...
export function validateStageManifest(manifest,requestedStage=manifest?.stage) {
  ...
  if(manifest?.schema_version!=='0.1'||!['M0','M1'].includes(manifest.stage)) return ['Unimplemented stage; only M0 and M1 have registered checks'];
  ...
  if(manifest.stage==='M1'&&!manifest.checks?.includes('integration'))errors.push('Missing mandatory check: integration');
  ...
  if(manifest.stage==='M1'&&!manifest.required_tools?.includes('oasdiff'))errors.push('Missing mandatory tool: oasdiff');
  return errors;
}
...
async function main(argv) {
  ...
  if(args.length!==2||args[0]!=='--stage') { console.error('Usage: pnpm verify:stage -- --stage M0|M1');return 64; }
  if(!['M0','M1'].includes(args[1])) { console.error('Unimplemented stage '+args[1]+': NOT VERIFIED');return 2; }
  const stage=args[1];
  const root=resolve('.');
  ...
  for(let index=1;index<=(stage==='M0'?9:27);index++) {
  ...
  await writeFile(path,JSON.stringify({schema_version:'0.1',stage,platform:process.platform+'-'+process.arch,runtime:'NOT_EXECUTED',...result},null,2)+'\n');
