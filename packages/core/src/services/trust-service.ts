...
import {resolveReviewedTool,reviewedPathDirectories,type ReviewedTool} from './reviewed-tool.js';
import {filterEnvironment} from '../domain/environment-filter.js';
import {readRunnerProvenance} from '../../../runner-local/src/provenance.js';
const digest=(value:unknown)=>hashBytes(Buffer.from(canonicalJson(value)));
export type ToolIdentity=ReviewedTool;
...
export function buildExecutionPreview(config:ProjectConfig,policy:ExecutionPermissions,tools:Record<string,ToolIdentity>){
  ...
  return {commands,tools,environment_names:[...config.security.env_allowlist].sort(),network_targets:[...new Set(policy.allowed_origins)].sort(),writable_paths:[...new Set(policy.allowed_paths)].sort(),docker_operations:Object.entries(config.environments).filter(([,e])=>e.mode==='compose').map(([id,e])=>({environment:id,operation:'compose up/down',configuration:e.mode==='compose'?e.compose_file:'',services:e.mode==='compose'?e.services:[]})),runtime:'NOT_EXECUTED' as const};
}
export class TrustService {
  ...
  private async snapshot(){
    ...
    const path_directories=await reviewedPathDirectories(this.root);
    const system_environment=filterEnvironment(process.env,[],process.platform);system_environment.PATH=path_directories.join(path.delimiter);
    const resolveTool=(exec:string,cwd:string)=>resolveReviewedTool(exec,cwd,this.root,path_directories);
    for(const [id,command] of Object.entries(config.commands)){
      ...
      for(const argument of command.args){
        ...
        try{const stat=await fs.stat(file);if(stat.isFile())scriptInputs.add(relative);else throw configurationError('REQUIRED_INPUT_UNVERIFIED: command directory or non-file input needs a concrete reviewed file entry point',id);}catch(error){if((error as NodeJS.ErrnoException).code!=='ENOENT')throw error;if(/\.(?:[cm]?[jt]sx?|py|sh|ps1|cmd|bat)$/i.test(value))throw configurationError('Script input is missing',id);}
      }
    }
    ...
    const inputScope=await executionInputScope(this.root,config,[...scriptInputs]);
    const manifest=await captureInputs({project_id:config.project_id,repo_root:this.root,configuration_hash,workspaces:config.workspaces,contracts:config.contracts},baseline,{exclusions:[{relative_path:config.state_dir,reason:'StackGate state'}],include_ignored:inputScope.include_ignored,required_files:inputScope.required_files,exclude_untracked_directory_names:generatedDirectoryNames});
    if(manifest.completeness!=='COMPLETE')throw configurationError('Execution inputs are incomplete');
    const analysis_tools={oasdiff:await new ToolRegistryService().resolveOasdiff()};
    const runner_provenance=await readRunnerProvenance();
    const execution_preview={...buildExecutionPreview(config,{allowed_origins:config.security.allow_origins,allowed_paths:[config.state_dir,...Object.values(config.workspaces).map(w=>w.path)]},tools),analysis_tools,runner_provenance,path_directories,system_environment,input_scope:inputScope.descriptor,repo_id:manifest.repo_id,worktree_id:manifest.worktree_id,platform_id:manifest.platform_id,configuration_hash,script_input_hash:digest(manifest.files)};
    return {execution_preview,execution_digest:digest(execution_preview),configuration:config,input_manifest:manifest};
  }
  async capture(){return this.snapshot();}
  async review(){
    const snapshot=await this.snapshot(),p={execution_preview:snapshot.execution_preview,execution_digest:snapshot.execution_digest},record=await this.store.read(p.execution_digest);
